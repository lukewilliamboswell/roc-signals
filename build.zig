...
///
/// All but `boundary` drive the engine as a state machine: they decode fuzzer
/// bytes into a valid program and check it against a slow reference model.
/// `boundary` is a conventional byte-oriented parser target.
