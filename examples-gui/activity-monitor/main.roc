...
view = |model, running, query, errors_only, selected, follow_tail| {
	...
	Gui.column(
		...
		[
			...
			Gui.row(
				[Gui.style({ ..Gui.style_default, gap: 16, width: Fill })],
				[
			...
			Gui.row(
				[Gui.style({ ..Gui.style_default, gap: 12, width: Fill, overflow_x: Clip })],
				[
					Gui.column([Gui.test_id("activity-status"), Gui.style({ ..Gui.style_default, grow: True, font_size: 13, foreground: Rgb(0xA9BFCC) })], [Gui.text_s(session.map(|state| state.notice))]),
				],
			),
			...
			Gui.row(
				...
				[
					Ui.when(
						...
						|| Gui.column(
							[Gui.style({ ..Gui.style_default, font_size: 12, foreground: Rgb(0xE8C27A) })],
							[Gui.text("Replay paused")],
						),
						|| Gui.text(""),
					),
					Gui.column(
						[Gui.style({ ..Gui.style_default, font_size: 12, foreground: Rgb(0xA9BFCC) })],
						[Gui.text_s(history.map(|value| "Retained: ${value.rows.len().to_str()} / 1000"))],
					),
					Gui.column(
						[Gui.style({ ..Gui.style_default, font_size: 12, foreground: Rgb(0x93A9B6) })],
			...
			Gui.column(
				[Gui.style({ ..Gui.style_default, grow: True, gap: 0, padding: 12, radius: 10, background: Rgb(0x1B2A33), width: Fill, overflow_x: Clip, overflow_y: Clip })],
				[
					Ui.when(
			...
			Gui.panel(
				...
				[
					Gui.row(
						...
						[
							Gui.column(
								[Gui.style({ ..Gui.style_default, width: Px(210), foreground: Rgb(0xA9BFCC), overflow_x: Clip })],
								[Gui.heading("Event inspector")],
							),
							Gui.column(
								[Gui.font_family(feed_font), Gui.test_id("inspector-detail"), Gui.style({ ..Gui.style_default, width: Fill, grow: True, font_size: 13 })],
								[
									Gui.text_s(
										inspection.map(
					...
					Ui.when(
						...
						|| Gui.row(
							...
							[
								...
								Gui.column([Gui.font_family(feed_font), Gui.style({ ..Gui.style_default, width: Fill, grow: True, font_size: 13 })], [Gui.text_s(session.map(|state| state.lines.partial))]),
							],
						),
						|| Gui.text(""),
					),
