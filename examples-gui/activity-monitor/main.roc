...
main : () -> Elem
main = || Ui.state(Feed.empty, |history| Ui.state(False, |running| Ui.state("", |query| Ui.state(False, |errors_only| Ui.state("", |selected| {
	...
	Gui.column([Gui.style({ ..Gui.style_default, padding: 24, gap: 16, width: Fill, height: Fill })], [
		Gui.heading("Activity Monitor"),
		Gui.text("SIMULATED REPLAY · deterministic sample operations, not system telemetry"),
		Gui.row([], [
			Gui.action_button({ label: running.signal().map(|active| if active { "Pause replay" } else { "Start replay" }), enabled: Signal.const(True) }, [], running.on_unit(|active| !active)),
		...
		Gui.row([Gui.style({ ..Gui.style_default, grow: True, width: Fill })], [
			Gui.panel([Gui.style({ ..Gui.style_default, grow: True, height: Px(480), overflow_y: Scroll }), Gui.test_id("activity-list")], [
			...
			Gui.panel([Gui.style({ ..Gui.style_default, width: Px(300), padding: 16 })], [
				Gui.heading("Event inspector"),
				Gui.text_s(inspection.map(|value| if value.selected.is_empty() {
