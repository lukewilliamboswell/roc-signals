				...
				Ui.state(
					...
					|confirm_discard| {
						...
						Gui.column([
							...
							Gui.text_input("Document title", title.signal(), title.on_str(|_, value| value)),
							Gui.text_input("Note text", body.signal(), body.on_str(|_, value| value)),
							Gui.card("note-summary", [Gui.text_s(counts.map(Document.counts_text))]),
							Gui.card(
								"note-status",
								[
									Gui.text_s(
