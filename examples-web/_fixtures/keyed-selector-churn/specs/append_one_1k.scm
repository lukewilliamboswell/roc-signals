(test "append one row at 1k registers only the new memberships"
  (steps
    ...
    (expect-metric-delta selector_key_bytes_copied 8)
    ...
    (mark-metrics)
    (click (role button :name "Select row 1001"))
    (expect-attr (test-id "row-3") class "")
    (expect-attr (test-id "pinned-3") class "")
    (expect-attr (test-id "row-1001") class "danger")
