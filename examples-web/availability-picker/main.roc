app [main] { roc: "nightly-2026-09-11-793f9d8", pf: platform "https://github.com/lukewilliamboswell/roc-signals/releases/download/0.2.0-rc2/AvyUxjkQaEPU7NKikDiz3U48XGMX1fpFgw2bppV87qLA.tar.zst" }
	...
	Html.section(
		...
		[
			...
			Html.div_c(
				...
				[
					Html.button_attrs("Available", [Html.aria_label("Mark ${key} available"), Html.class_attr("button button-sm")], slots.update(|list| set_status(list, key, Status.Available))),
					Html.button_attrs("Busy", [Html.aria_label("Mark ${key} busy"), Html.class_attr("button button-sm")], slots.update(|list| set_status(list, key, Status.Busy))),
					Html.button_attrs("Earlier", [Html.aria_label("Move ${key} earlier"), Html.class_attr("button-ghost button-sm")], slots.update(|list| move_earlier(list, key))),
					Html.button_attrs("Remove", [Html.aria_label("Remove ${key}"), Html.class_attr("button-danger button-sm")], slots.update(|list| list.drop_if(|slot| slot.id == key))),
				],
			),
		],
	)
						...
						[
							field(
								...
								Html.text_input_attrs(
									...
									draft.update_str_with(zone, |value, current_zone, text| reprice({ ..value, title: text }, current_zone)),
								),
								"Shown on the block.",
							),
							field(
								...
								Html.select_c(
									...
									draft.update_str_with(zone, |value, current_zone, text| reprice({ ..value, day: Day.from_str(text).ok_or(Day.Mon) }, current_zone)),
								),
							...
							field(
								...
								Html.text_input_attrs(
									...
									draft.update_str_with(zone, |value, current_zone, text| reprice({ ..value, start_text: text }, current_zone)),
								),
								"24-hour HH:MM.",
							),
							field(
								...
								Html.text_input_attrs(
									...
									draft.update_str_with(zone, |value, current_zone, text| reprice({ ..value, duration: text }, current_zone)),
								),
								"Minutes, 1 to 720.",
							),
						],
					...
					Html.div_c(
						...
						[
							...
							Html.action_button_attrs(
								...
								slots.update_with(draft, add_slot),
							),
						],
					),
				],
			...
			Html.div_c(
				...
				[
					...
					Html.select_c(
						...
						zone.update_str(|_, text| zone_by_id(text)),
					),
