app [main] { pf: platform "https://github.com/lukewilliamboswell/roc-signals/releases/download/0.2.0-rc2/AvyUxjkQaEPU7NKikDiz3U48XGMX1fpFgw2bppV87qLA.tar.zst", roc: "nightly-2026-09-12-220fd47" }
			...
			Html.div_c(
				...
				[
					...
					Html.text_input_attrs(
						...
						capture.update_str(|current, text| Notes.edit_note(current, key, Notes.sanitize(text))),
					),
				],
			),
			...
			Html.div_c(
				...
				[
					Html.action_button_c(
						...
						"button button-sm",
						capture.update(|current| Notes.push_op(current, OpQueue(key))),
					),
					Html.action_button_attrs(
						...
						capture.update(|current| Notes.retry_note(current, key)),
					),
					Html.button_c("Promote note ${key}", "button-ghost button-sm", capture.update(|current| Notes.push_op(current, OpPromote(key)))),
					Html.button_c("Delete note ${key}", "button-ghost button-sm", capture.update(|current| Notes.push_op(current, OpDelete(key)))),
				],
			),
		],
	)
											...
											Html.div_c(
												...
												[
													Html.div_c(
														toolbar_class,
														[
															check_row("Sync automatically", auto_sync.signal(), auto_sync.update_bool(|_current, checked| checked)),
															check_row("Hide synced notes", hide_synced.signal(), hide_synced.update_bool(|_current, checked| checked)),
														],
													),
													Html.div_c(
														toolbar_class,
											...
											Html.div_c(
												...
												[
													...
													Html.textarea_attrs(
														...
														capture.update_str(|current, text| { ..current, draft: text }),
													),
												],
											),
											Html.action_button_c(Signal.const("Save note"), save_disabled, "button-primary justify-self-start", capture.update(Notes.save_note)),
										],
									),
									Html.section_c(
										"Outbox",
