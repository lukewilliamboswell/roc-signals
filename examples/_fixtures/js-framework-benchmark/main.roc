...
make_rows = |start, count|
	List.repeat(0.U64, count).map_with_index(|_, index| {
...
update_every_tenth = |model| {
	rows = model.rows.map_with_index(|row, index|
		if index % 10 == 0 { { ..row, label: "${row.label} !!!" } } else { row })
	{ ..model, rows }
}
...
render_row : Ui.State(Model), Str, Signal.Signal(Row) -> Elem
render_row = |model, key, row| {
	label = row.map(|value| value.label)
	classes = model.signal().map(|value| if value.selected == key { "danger" } else { "" })
	element("tr", [Html.class_attr_s(classes), Html.attr("data-row-id", key), Html.test_id("row-${key}")], [
		element("td", [Html.class_attr("col-md-1")], [Html.text(key)]),
		element("td", [Html.class_attr("col-md-4")], [
		...
		element("td", [Html.class_attr("col-md-1")], [
...
main : () -> Elem
main = ||
	Ui.state({ rows: [], next_id: 1.U64, selected: "" }, |model| {
		rows = model.signal().map(|value| value.rows)
		Html.div([Html.class_attr("container")], [
			Html.heading("roc-signals keyed"),
			Html.section("Benchmark controls", [], [
			...
			element("table", [Html.class_attr("table table-hover table-striped test-data")], [
				element("tbody", [], [Ui.each_str(rows, |row| row.id.to_str(), |key, row| render_row(model, key, row))]),
			]),
		])
	})
