...
billing_label = |value| Str.concat("Billing cadence: ", value)

changes_label : I64 -> Str
changes_label = |value| Str.concat("Changes: ", value.to_str())
...
main = |_| {
	Ui.state(
		...
		|model| {
			...
			billing_signal = Signal.map(state_signal, |state| state.billing)
			billing_text = Signal.map(billing_signal, billing_label)
