(test "Pomodoro tracker — start: the running branch mounts the interval"
  ...
  (steps
    ...
    (expect-text (test-id "row-total-docs") "Docs pass: 11 min today")
    (expect-text (test-id "focus-minutes") "Focus minutes today: 16")
    (expect-text (test-id "blocks-logged") "Blocks logged today: 3")
    ...
    (expect-text (test-id "row-total-docs") "Docs pass: 12 min today")
