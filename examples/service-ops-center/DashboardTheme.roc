DashboardTheme :: [].{
	...
	toolbar_class : Str
	toolbar_class = "grid gap-3 border-b border-zinc-200 bg-white px-4 py-3 sm:flex sm:flex-wrap sm:items-center sm:justify-between sm:px-5"

	toolbar_text_class : Str
	toolbar_text_class = "grid gap-1"

	toolbar_status_class : Str
	toolbar_status_class = "break-words text-sm text-zinc-600"

	toolbar_actions_class : Str
	toolbar_actions_class = "grid gap-3 text-sm text-zinc-600 sm:flex sm:flex-wrap sm:items-center"
	...
	metric_detail_class = "break-words text-sm leading-5"
	...
	row_bar_class : Str
	row_bar_class = "text-xs font-medium text-zinc-600"

	row_bar_shell_class : Str
	row_bar_shell_class = "h-2 w-full overflow-hidden rounded bg-zinc-200"

	segment_fill_class : Str -> Str
	segment_fill_class = |label|
		if label == "3/12 segments" {
			"h-2 rounded bg-emerald-600 w-[25%]"
		} else if label == "4/12 segments" {
			"h-2 rounded bg-emerald-600 w-[33%]"
		} else if label == "5/12 segments" {
			"h-2 rounded bg-emerald-600 w-[42%]"
		} else if label == "6/12 segments" {
			"h-2 rounded bg-emerald-600 w-[50%]"
		} else if label == "7/12 segments" {
			"h-2 rounded bg-emerald-600 w-[58%]"
		} else if label == "8/12 segments" {
			"h-2 rounded bg-emerald-600 w-[67%]"
		} else if label == "9/12 segments" {
			"h-2 rounded bg-amber-500 w-[75%]"
		} else if label == "10/12 segments" {
			"h-2 rounded bg-amber-500 w-[83%]"
		} else {
			"h-2 rounded bg-red-500 w-[92%]"
		}

	row_note_class : Str
	row_note_class = "break-words text-sm text-zinc-600"

	traffic_row_class : Str
	traffic_row_class = "grid gap-1 border-t border-zinc-100 py-2 sm:grid-cols-5 sm:gap-3"
	...
	metric_class = |tone| "grid min-h-32 min-w-0 gap-2 rounded-md border p-4 ${tone_panel_class(tone)}"
