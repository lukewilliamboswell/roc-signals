(test "Spreadsheet lite — initial mount"
  (steps
    ...
    (expect-text (test-id "status-line") "Selected: A1 | Grid mode: values | Errors: 4")
    ; Formula bar for the initially selected cell.
    (expect-value (label "Formula") "Item")
    (expect-text (test-id "bar-cell") "Cell: A1")
    (expect-text (test-id "bar-source") "Source: Item")
    (expect-text (test-id "bar-value") "Value: Item")
    (expect-text (test-id "bar-depends") "Depends on: none")
