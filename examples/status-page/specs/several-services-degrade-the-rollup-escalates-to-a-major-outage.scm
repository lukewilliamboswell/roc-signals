(test "Status page — Several services degrade: the rollup escalates to a major outage"
  (steps
    ...
    (expect-text (test-id "status-breakdown") "Operational 1, degraded 3, outage 0, awaiting 0")
    (resolve-task "check:notifications" "operational|99.95")
    (expect-text (test-id "overall-rollup") "Major outage")
    (expect-text (test-id "overall-uptime") "Overall uptime: 97.86%")
    ...
    (expect-text (test-id "open-incident-count") "Open incidents: 2")
    (expect-visible (role region :name "Incident inc-51"))
    (expect-text (test-id "incident-inc-51-severity") "inc-51 severity: Minor")
    (expect-text (test-id "incident-inc-51-latest") "inc-51 latest: 11:10 - Investigating a backlog")
  )
)
