(test "Token editor — boundary: a spacing token of zero"
  (steps
    ...
    (expect-text (test-id "preview-surface") "color: #767676; background: #ffffff; padding: 0px; font-size: 16px")
    (expect-text (test-id "preview-button") "color: #ffffff; background: #2563eb; padding: 0px; font-size: 16px")
    (expect-text (test-id "preview-badge") "color: #ffffff; background: #767676; padding: 0px; font-size: 16px")
    ...
    (expect-text (test-id "preview-surface") "color: #767676; background: #ffffff; padding: /* invalid */; font-size: 16px")
    (expect-text (test-id "ratio-text") "4.54:1")
    (expect-text (test-id "aa-summary") "2 of 2 pairs pass AA")
    (fill (label "Spacing small") "8")
    (expect-text (test-id "token-validity") "All 6 tokens valid")
    (expect-text (test-id "preview-surface") "color: #767676; background: #ffffff; padding: 8px; font-size: 16px")
  )
)
