...
and Explorer were launched with `--assets-root` pointing at their `assets/`.
...
The Linux diagnostics in [../2026-09-10/repros](../2026-09-10/repros/) were
replayed against the Windows executables with `--run-spec-json --entropy-seed 0`
and fail identically. No new Windows spec could be written: the fixture guard
(`src/spec/file_fixtures.zig`, `validPath`) still rejects any path that does
not begin with `/`, so the typed fixtures cannot express `C:\…`.
