...
## Breaking changes and migration: 0.1.1 → 0.2.0-rc1
...
`python3 scripts/bundle_browser.py`. Production deployment uses the exact tested
release archive, paired runtime, and complete `signals-starters.zip` sources.
Downloadable examples pin the immutable GitHub release URL. The supported site
also retains versioned documentation and existing platform download URLs.
