...
pub const TextField = enum(u64) {
    ...
    pub fn isNative(self: TextField) bool {
        return switch (self) {
            .native_style, .native_viewport, .native_drag_key, .native_window_close, .native_placeholder => true,
            else => false,
        };
    }
    ...
    pub fn setOp(self: TextField) Op {
        return switch (self) {
            ...
            .value => .set_value,
            .class => .set_class,
            .native_style, .native_viewport, .native_drag_key, .native_window_close, .native_placeholder => @panic("native text metadata has no browser opcode"),
        };
    }
};
...
test "every native scalar counts as metadata without a browser opcode" {
    ...
    try std.testing.expectEqual(@as(u64, 7), counts.total);
