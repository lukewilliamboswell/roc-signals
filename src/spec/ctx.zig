...
pub const Capabilities = struct {
    ...
    window: bool = false,
    /// The pending task registry knows each task's kind, so a typed file
    /// fixture can be refused before its payload is decoded.
    fixture_admission: bool = false,
...
pub const required = [_][]const u8{
    ...
    "resolvePendingTask",                "resolveStalePendingTask", "tickIntervalSource",
    "navigateLocation",                  "historyBack",             "historyForward",
    "currentLocation",                   "finishHostMetrics",       "lastRuntimeMetrics",
    "cleanupEventCount",                 "pendingTaskCountByName",  "canceledTaskCountByName",
    "activeIntervalRecordCountByPeriod",
};
...
const fixture_admission_decls = [_][]const u8{"pendingTaskKind"};
...
pub fn assertRunnerCtx(comptime Ctx: type) void {
    comptime {
        ...
        if (caps.fixture_admission) for (fixture_admission_decls) |name| requireFn(Ctx, name, "the fixture_admission capability");
