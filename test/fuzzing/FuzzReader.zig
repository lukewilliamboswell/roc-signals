...
//!
//! Fuzz targets in this directory are *generators*: they turn an arbitrary byte
//! string from AFL++ into a structured, always-valid program (a signal graph, an
//! operation sequence, a boundary descriptor) and then assert engine invariants
//! against a reference model. This reader is the single decoding seam they share.
//!
//! Reads past the end of the input yield zero rather than failing, so every byte
//! string decodes to *some* program. That keeps AFL++'s mutations cheap: it can
//! truncate or extend an input without the target rejecting it outright, and a
//! short input simply decodes to a short program.
...
/// Wraps a fuzzer-supplied byte string for structured decoding.
...
/// Consumes one byte, returning zero once the input is exhausted.
...
/// Consumes one byte and reads its low bit as a boolean choice.
pub fn boolean(self: *Self) bool {
    return (self.readByte() & 1) != 0;
}

/// Consumes enough bytes to pick a value in `[min, max]` inclusive.
...
/// Consumes enough bytes to pick a value in `[min, max)`.
